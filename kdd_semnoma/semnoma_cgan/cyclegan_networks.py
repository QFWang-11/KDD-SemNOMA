...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        # 从最内层构造U-Net结构，创建UnetSkipConnectionBlock作为最内层
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if type(norm_layer) == functools.partial:
            ...
            use_bias = norm_layer == nn.InstanceNorm2d
        if input_nc is None:  #没有最初的input_nc就等于上一个conv的输出
            input_nc = outer_nc
        ...
        self.upsamples = nn.Sequential(UpsampleBlock(outer_nc, 2), UpsampleBlock(outer_nc, 2))
    ...
    def forward(self, x):
        if self.outermost:
            ...
            concatenated = torch.cat([x, out], 1)
...
'''
image = torch.randn(1,64,32,32) #输入特征，输出特征U-Net维度相同
csi = torch.randn(1,1)
Generator = define_G(input_nc=64, output_nc=6, ngf=64, netG='unet_32', use_dropout=False, norm='none')
out = Generator(image)
# Final upsampling to (batch, 6, 256, 256)
out = nn.functional.interpolate(out, scale_factor=8, mode='bilinear', align_corners=True)
print('G_out',out.size())
'''
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.cam_blocks = nn.Sequential(*[CAMBlock(128) for _ in range(7)])
