...
import Acnet_gan
from kd_semnoma.datasets.ffgq256_group_wrapper import dataloaders
...
parser.add_argument('--net_G', type=str, default='unet_32', metavar='str',
...
parser.add_argument('--enable_d1d2', action='store_true', default=False, # action='store_true': specifies this parameter's behavior. When user provides --enable_d1d2 option in command line, enable_d1d2 will be set to True. If user doesn't provide this option, enable_d1d2 will remain False by default
                    help='imporve the G by adversarial training')
parser.add_argument('--enable_d3', action='store_true', default=False,
                    help='imporve the G by adversarial training')        # adversarial training on image patches
...
parser.add_argument('--lambda_fea', type=float, default=0.1,        # loss weight selection affects PSNR and LPIPS performance metrics
                    help='coefficient on fea loss to balance with other losses')
...
if __name__ == '__main__':
    ...
    ac = Acnet_gan.Acnet_GAN(args=args, dataloaders=dataloaders)
    ac.train_models()
