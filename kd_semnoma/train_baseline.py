...
import numpy as np
import datetime
from models.losses import Sum_MSE, MSE, Average_PSNR, PSNR
...
def train(net, EPOCH, train_loader, valid_loader, checkpoint_path='checkpoint_baseline_M_32_2user_Rayleigh_FFHQ256_-1010dB.pth'):
    # optimizer
    ...
    best_loss = 200
    # Training
    writer = SummaryWriter('./path_2users/to/log_monitor_baseline_M_32_2user_Rayleigh_FFHQ256_-1010dB')
    ...
    for epoch in range(start_epoch, EPOCH):
        ...
        if loss_val < best_loss:
            best_loss = loss_val
            torch.save(net.state_dict(), 'UL_NOMA_2_Users_baseline_M_32_Rayleigh_FFHQ256_-1010dB.pth')
            print('model saved')
...
num_devices = 2
M = 32
#net_hkd = SingleModelNet_A1_HKD(num_devices=num_devices, M=M).cuda()
net_baseline = SingleModelNet(num_devices=num_devices, M=M).cuda()
EPOCH = 1000
train(net_baseline, EPOCH, train_loader, valid_loader)