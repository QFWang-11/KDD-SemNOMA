...
class readImageDataset(Dataset):
	def __init__(self, state, input_transform = None):
		self.state = state
		self.root = r'D:\wqf\FFHQ\FFHQ_256'       
	...
	def getDataPath(self):
		self.train_input_paths = glob(self.root + r'\train\*')
		self.val_input_paths = glob(self.root + r'\val\*')
...
class GroupingWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.group_size):
            x = self.dataset.__getitem__(indices[i])
            X.append(x)
            #if cur_y == 1:
            #    y_single[i] = 1
...
def getArgs():
    ...
    parse.add_argument("--group_size", type=int, default= 2)
    ...
    parse.add_argument("--num_train_samples", type=int, default=50000)
...
csi_val = generate_noise_vec(len(val_dataset), args)
#csi_test = generate_noise_vec(len(test_data), args)
train_loader = train_dataloader(args, train_dataset, csi_train)
valid_loader = val_dataloader(args, val_dataset, csi_val)
test_loader = test_dataloader(args, val_dataset)